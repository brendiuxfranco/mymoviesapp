<template>
  <v-card>
    ...
    <v-card-subtitle class="pb-0">
      <v-chip v-for="genre in movie.genre_ids" :key="genre" class="ma-2 pink--text text--darken-4" color="pink lighten-4" x-small>
    ...
    <v-card-actions>
      <v-btn color="dark pink" text :disabled="noLoved(movie.id)" @click="addLovedMovie(movie)">
        Love it
      </v-btn>
      <v-spacer />
      <v-btn color="dusty pink" :disabled="noWatchlist(movie.id)" text @click="addWatchlist(movie)">
        Watchlist
      </v-btn>
    </v-card-actions>
  </v-card>
